...
export default function MyOrders({navigation,route}) {
  // let {reviews } = route.params
//   const renderplacedList = () => (
//     reviews.map((review,index)=>(
//      (review.status == "placed" ?  <Card>
//              <Text >{ review.title }</Text>
//              <Button title="Take Order" onPress={() =>{ review.status="finished"; navigation.navigate('Finished',{reviews});  const newReview =[...reviews];newReview.splice(newReview.indexOf(review),1);
...
const renderList = () => (
  orders.map((order,index)=>(
   (order.status == "confirmed" ?  <Card>
           ...
           <Button title="finish Order" onPress={() =>{ navigation.navigate('Finished') }}/>
          </Card> :<></>
      )
))
)
